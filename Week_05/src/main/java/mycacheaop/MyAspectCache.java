...
import mycacheaop.cacheimpl.CacheUtils;
...
public class MyAspectCache {
     ...
     * 定义切入点
     */
    @Pointcut("@annotation(mycacheaop.MyCache)")
    private void cut() {
        // do nothing
    }
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 在方法执行前判断是否存在缓存
        Object object = CacheUtils.getInstance().getCache(strKey, myCache);
        if (object == null) {
            // 创建缓存
    ...
    private Object createCache(ProceedingJoinPoint joinPoint, String strKey, MyCache myCacheable) throws Throwable {
        ...
        CacheUtils.getInstance().putKey(strKey,object,myCacheable);
        return object;
    }
}
